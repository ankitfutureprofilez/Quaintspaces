...
export default function payment({record}) {
  ...
  useEffect(() => {
    ...
    const response = main.all_user_payment_history(record);
    response
      .then((res) => {
              ...
              {content && content.map((item, index) => (
                <tr className="" key={index}>
                  ...
                  <td className="whitespace-no-wrap py-4 px-6 text-right text-sm text-gray-600 lg:text-left">
                    {item?.price}
                    <div className="flex mt-1 ml-auto w-fit items-center rounded-full bg-blue-600 py-2 px-3 text-left text-xs font-medium text-white lg:hidden">{item?.payment_status}</div>
                  </td>
